...
__global__ void reset(float4* pos, float4* d_pos_sorted, int* d_start, int* d_nodes, int* d_index, int* d_count, int* d_validBodies, int* d_validBodiesTop, int n, int m);
...
__global__ void compute_force(int* d_idx_to_body, int* tree, int* count, float4* pos, float4* sorted_pos, float4* acc, float4* d_bounds, int n, int m, const float SOFTENING);